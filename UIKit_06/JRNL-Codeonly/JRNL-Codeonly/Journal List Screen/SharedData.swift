...
class SharedData {
    ...
    func loadJournalEntriesData() {
        let fileURL = getDocumentDirectory().appendingPathComponent("journaEntriesData.json")
        
        do {
            let data = try Data(contentsOf: fileURL)
            let journalEntriesData = try
            
            JSONDecoder().decode([JournalEntry].self, from: data)
            journalEntries = journalEntriesData
        } catch {
            print("Failed to rea JSON data: \(error.localizedDescription)")
        }
    }
